...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string; closingId: string }> }) {
  ...
  if (coworkingId == null || closingId == null) return NextResponse.json({ message: 'Invalid id.' }, { status: 400 });
  try {
    await archivePlaceClosing(session.token, coworkingId, closingId);
    return new NextResponse(null, { status: 204 });
  } catch (error) {
    if (error instanceof BackendRequestError) return NextResponse.json({ message: error.message }, { status: error.status || 500 });
    return NextResponse.json({ message: 'Unable to archive place closing.' }, { status: 500 });
  }
}
