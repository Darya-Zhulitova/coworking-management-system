...
import { BackendRequestError, previewDeactivatePlace } from '@/lib/api/backend';
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string; placeId: string }> }) {
  ...
  try {
    return NextResponse.json(await previewDeactivatePlace(session.token, coworkingId, parsedPlaceId));
  } catch (error) {
    if (error instanceof BackendRequestError) return NextResponse.json({ message: error.message }, { status: error.status || 500 });
    return NextResponse.json({ message: 'Не удалось подготовить предпросмотр деактивации места.' }, { status: 500 });
  }
}
