...
import { approvePayRequest, BackendRequestError } from '@/lib/api/backend';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; payRequestId: string }> }) {
  ...
  try {
    ...
    await approvePayRequest(session.token, coworkingId, parsedId, comment && comment.length > 0 ? comment : undefined);
