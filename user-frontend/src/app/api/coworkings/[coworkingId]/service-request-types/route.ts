import { getServiceRequestTypes } from '@/lib/api/backend';
...
export async function GET(_request: Request, { params }: { params: Promise<{ coworkingId: string }> }) {
  ...
  try {
    return okJson(await getServiceRequestTypes(parsedCoworkingId.value, session.value.token));
  } catch (error) {
    return handleApiError(error, 'Unable to load service request types.');
  }
}
